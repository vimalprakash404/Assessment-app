...
function dataPaster(user, number, doc) {
    ...
    doc.setFontSize(20);
    doc.text("URL: " + "example.com", 65, number + 50);
    doc.text("UN: " + user.username+"@example.com", 65, number + 60);
    doc.text("PW: " + user.password, 65, number + 70);
    doc.rect(199.1, number + 20, 10, 60);
    doc.text("ROLL NO: " + user.username, 207, number + 77, null, 90);
    doc.line(0, number + 90, 250, number + 90);
}

export function createPDF(users) {
    const doc = new jsPDF({
        orientation: "p",
        unit: "mm",
        format: "a4"
